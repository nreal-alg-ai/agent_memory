...
RSYNC_ARGS=(
    ...
    --exclude='*.pyc'
    --exclude=.DS_Store
    --exclude=sync_qwen_audio_agent_memory.sh
)
...
# is outside these paths and is never touched.  Exclude this sync script so
# the vendored worktree keeps its own copy unchanged.
